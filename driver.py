...
def api_tests():
    ...
    register_node(server1, server2)  # server1 node will be register inside server2

    data = {'name': 'Maclachlan', 'firstName': 'Alasdair', 'address' : 'Cologne', 'score' : '0'}
    create_transaction(server2, {'sender': 'myWallet', 'recipient': '0', 'data': data})

    mine_block(server2)  # Mined a new block on server2

    sync_chain(server1)  # updating server1's chain with neighbour node's chain

    create_transaction(server1, {'sender': 'companyWallet', 'recipient': 'myWallet', 'data': {'score' : '1'}})
    mine_block(server1)  # Mined a new block on server1


    sync_chain(server2)  # updating server2's chain with neighbour node's chain
